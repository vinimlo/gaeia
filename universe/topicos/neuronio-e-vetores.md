...
- **Karpathy - Neural Networks: Zero to Hero (inicio do micrograd)** - Primeiro contato com neuronio computacional (assista os primeiros ~30 min)
...
**Um neuronio e um classificador linear.** O dot product `w·x` mede o quanto o input se alinha com os pesos - geometricamente, e uma projecao. O bias desloca a fronteira de decisao, e a sigmoid "esmaga" o resultado em uma probabilidade entre 0 e 1. Com um neuronio, voce so consegue separar dados com uma linha reta. Para resolver problemas nao-lineares como XOR, voce precisa empilhar neuronios - que e exatamente o proximo topico.

---

## Entregavel

Um neuronio em NumPy que recebe inputs, multiplica por pesos, soma bias, aplica sigmoid, e retorna output.

**Sanity check:** Com inputs `[1, 0.5]`, pesos `[0.7, -0.3]`, bias `0.1`, seu neuronio deve retornar `~0.5987`.
